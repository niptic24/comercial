...
$stmt = $conn->prepare('SELECT SUM(venta) as totalg FROM hosp where asesor =?');
$stmt->execute([$_SESSION['asesor']]);
$row = $stmt->fetch(PDO::FETCH_ASSOC);
$venta_global_diab_c = $row['totalg'];

//Obtener valor presupuesto global
$stmtp = $conn->prepare('SELECT SUM(presupuesto) as totalgp FROM hosp where asesor =?');
$stmtp->execute([$_SESSION['asesor']]);
$rowp = $stmtp->fetch(PDO::FETCH_ASSOC);
$presupuesto_global_hosp_c = $rowp['totalgp'];
...
//años
$año=date("Y");
$stmt1 = $conn->prepare('SELECT SUM(venta) as totalm FROM hosp where asesor =? and mes=? and ano=?');
$stmt1->execute([$_SESSION['asesor'],$mes,$año]);
$row1 = $stmt1->fetch(PDO::FETCH_ASSOC);
$venta_mensual_hosp_c = $row1['totalm']; 

//Obtener valor presupuesto mensual
$stmtp1 = $conn->prepare('SELECT SUM(presupuesto) as totalpm FROM hosp where asesor =? and mes=? and ano=?');
$stmtp1->execute([$_SESSION['asesor'],$mes,$año]);
$rowp1 = $stmtp1->fetch(PDO::FETCH_ASSOC);
$presupuesto_mensual_hosp_c = $rowp1['totalpm']; 

//obtener grafica venta mensual
$statement = $conn->prepare("SELECT * FROM hosp  WHERE asesor = ? and ano = 2022 GROUP BY mes ASC; ");
$statement->execute([ $_SESSION['nusuario']]);
$user = $statement 	-> fetchAll(PDO::FETCH_ASSOC);
$data1 = '';
	$data2 = '';
...
$data1 = trim($data1,",");
  ...
  if($venta_mensual_hosp_c != 0 and $presupuesto_mensual_hosp_c!= 0 ){
    $porcentajeventabarra= round( $venta_mensual_hosp_c / $presupuesto_mensual_hosp_c * 100);
  }
$porcentaje1=$presupuesto_mensual_hosp_c*70/100;
$porcentaje2=$presupuesto_mensual_hosp_c*95/100;
$porcentaje3=$presupuesto_mensual_hosp_c*100/100;            
$respuesta = "";
$textom = "";  
 
if ($venta_mensual_hosp_c < $porcentaje3 ) {
 $respuesta = $porcentaje3 - $venta_mensual_hosp_c; 
 $textom = "<font color=\"red\">Falta $".$respuesta." para llegar al valor del presupuesto mensual.</font>";
 } else if ($venta_mensual_hosp_c >= $porcentaje3) {
   $respuesta = $venta_mensual_hosp_c-$porcentaje3; 
 $textom = "<font color=\"green\">Superaste por $".$respuesta." el valor del presupuesto mensual.</font>";
  ...
  if($venta_global_diab_c != 0 and $presupuesto_global_hosp_c!= 0 ){
    $porcentajeventabarrag= round( $venta_global_diab_c / $presupuesto_global_hosp_c* 100);
  }
$porcentajeg1=$presupuesto_global_hosp_c*70/100;
$porcentajeg2=$presupuesto_global_hosp_c*95/100;
$porcentajeg3=$presupuesto_global_hosp_c*100/100;            
$respuestag = "";
$textog = "";  
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="h5 mb-0 font-weight-bold text-gray-800"><?php	echo "$".$presupuesto_global_hosp_c ;  ?></div>
                                        </div>
                                        <div class="col-auto">
                                            <i class="fas fa-dollar-sign fa-2x text-gray-300"></i>
                                        </div>
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="row no-gutters align-items-center">
                                                <div class="col-auto">
                                                    <div class="h5 mb-0 mr-3 font-weight-bold text-gray-800"><?php 	echo "$".$venta_mensual_hosp_c ?></div>
                                                </div>
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="h5 mb-0 font-weight-bold text-gray-800"> <?php echo "$".$presupuesto_mensual_hosp_c ;?></div>
                                        </div>
                                        <div class="col-auto">
                                            <i class="fas fa-dollar-sign fa-2x text-gray-300"></i>
                                        </div>
...
var myPieChart = new Chart(ctx, {
  ...
  data: {
    ...
    datasets: [{
      data: [<?php echo $venta_mensual_hosp_c ?>,<?php echo $presupuesto_mensual_hosp_c ?>],
...
var myPieChart = new Chart(ctx, {
  ...
  data: {
    labels: ["Venta", "Presupuesto"],
    datasets: [{
      data: [<?php echo $venta_global_diab_c ?>,<?php echo $presupuesto_global_hosp_c ?>],
      backgroundColor: ['#4e73df', '#1cc88a', '#36b9cc'],
      hoverBackgroundColor: ['#2e59d9', '#17a673', '#2c9faf'],
      hoverBorderColor: "rgba(234, 236, 244, 1)",
    }],
