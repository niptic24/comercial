...
$stmt = $conn->prepare('SELECT SUM(venta) as totalg FROM asp where asesor =?');
$stmt->execute([$_SESSION['asesor']]);
$row = $stmt->fetch(PDO::FETCH_ASSOC);
$venta_global_asp_g = $row['totalg'];

//Obtener valor presupuesto global
$stmtp = $conn->prepare('SELECT SUM(presupuesto) as totalgp FROM asp where asesor =?');
$stmtp->execute([$_SESSION['asesor']]);
$rowp = $stmtp->fetch(PDO::FETCH_ASSOC);
$presupuesto_global_asp_g = $rowp['totalgp'];
...
$statement = $conn->prepare("SELECT  mes,venta FROM `asp` WHERE `asesor`=? and `ano` =?");
...
//mes año actual
$statement1 = $conn->prepare("SELECT mes, venta FROM `asp` WHERE `asesor`=? and `ano` =?");
...
$stmt1 = $conn->prepare('SELECT SUM(venta) as totalm FROM asp where asesor =? and mes=? and ano=?');
$stmt1->execute([$_SESSION['asesor'],$mes,$año]);
$row1 = $stmt1->fetch(PDO::FETCH_ASSOC);
$venta_mensual_aspg = $row1['totalm']; 

//Obtener valor presupuesto mensual

$stmtp1 = $conn->prepare('SELECT SUM(presupuesto) as totalpm FROM asp where asesor =? and mes=? and ano=?');
$stmtp1->execute([$_SESSION['asesor'],$mes,$año]);
$rowp1 = $stmtp1->fetch(PDO::FETCH_ASSOC);
$presupuesto_mensual_aspg = $rowp1['totalpm']; 

//obtener grafica venta mensual
$statement = $conn->prepare("SELECT * FROM asp  WHERE asesor = ? and ano = 2022 GROUP BY mes ASC; ");
$statement->execute([ $_SESSION['nusuario']]);
$user = $statement 	-> fetchAll(PDO::FETCH_ASSOC);
$data1 = '';
	$data2 = '';
...
$data1 = trim($data1,",");
 ...
 if($venta_mensual_aspg != 0 and $presupuesto_mensual_aspg!= 0 ){
   $porcentajeventabarra= round( $venta_mensual_aspg / $presupuesto_mensual_aspg * 100);
 }
$porcentaje1=$presupuesto_mensual_aspg*70/100;
$porcentaje2=$presupuesto_mensual_aspg*95/100;
$porcentaje3=$presupuesto_mensual_aspg*100/100;            
$respuesta = "";
$textom = "";  

if ($venta_mensual_aspg < $porcentaje3 ) {
$respuesta = $porcentaje3 - $venta_mensual_aspg; 
$textom = "<font color=\"red\">Falta $".$respuesta." para llegar al valor del presupuesto mensual.</font>";
} else if ($venta_mensual_aspg >= $porcentaje3) {
  $respuesta = $venta_mensual_aspg-$porcentaje3; 
$textom = "<font color=\"green\">Superaste por $".$respuesta." el valor del presupuesto mensual.</font>";
 ...
 if($venta_global_asp_g != 0 and $presupuesto_global_asp_g!= 0 ){
   $porcentajeventabarrag= round( $venta_global_asp_g / $presupuesto_global_asp_g* 100);
 }
$porcentajeg1=$presupuesto_global_asp_g*70/100;
$porcentajeg2=$presupuesto_global_asp_g*95/100;
$porcentajeg3=$presupuesto_global_asp_g*100/100;            
$respuestag = "";
$textog = "";  

if ($venta_global_asp_g < $porcentajeg3 ) {
 $respuestag = $porcentajeg3 - $venta_global_asp_g; 
 $textog = "<font color=\"red\">Falta $".$respuestag." para llegar al valor del presupuesto global.</font>";
 } else if ($venta_global_asp_g >= $porcentajeg3) {
   $respuestag = $venta_global_asp_g-$porcentajeg3; 
...
<body id="page-top">
    ...
    <div id="wrapper">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Linea ASP </h1>
                        </div>
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="h5 mb-0 font-weight-bold text-gray-800"><?php	echo "$".$presupuesto_global_asp_g ;  ?></div>
                                        </div>
                                        <div class="col-auto">
                                            <i class="fas fa-dollar-sign fa-2x text-gray-300"></i>
                                        </div>
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="row no-gutters align-items-center">
                                                <div class="col-auto">
                                                    <div class="h5 mb-0 mr-3 font-weight-bold text-gray-800"><?php 	echo "$".$venta_mensual_aspg ?></div>
                                                </div>
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="h5 mb-0 font-weight-bold text-gray-800"> <?php echo "$".$presupuesto_mensual_aspg ;?></div>
                                        </div>
                                        <div class="col-auto">
                                            <i class="fas fa-dollar-sign fa-2x text-gray-300"></i>
                                        </div>
                        ...
                        <div class="col-xl-8 col-lg-7">
                            <div class="card shadow mb-4">
                                ...
                                <div
                                    ...
                                    <h6 class="m-0 font-weight-bold text-primary">Semaforización  del mes  <?php echo date('M'), ' del ' , $año?></h6>
...
var myPieChart = new Chart(ctx, {
  ...
  data: {
    ...
    datasets: [{
      data: [<?php echo $venta_mensual_aspg ?>,<?php echo $presupuesto_mensual_aspg?>],
...
var myPieChart = new Chart(ctx, {
  ...
  data: {
    labels: ["Venta", "Presupuesto"],
    datasets: [{
      data: [<?php echo $venta_global_asp_g ?>,<?php echo $presupuesto_global_asp_g ?>],
      backgroundColor: ['#4e73df', '#1cc88a', '#36b9cc'],
      hoverBackgroundColor: ['#2e59d9', '#17a673', '#2c9faf'],
      hoverBorderColor: "rgba(234, 236, 244, 1)",
    }],
    ...
    <div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">Sele "Logout" below if you are ready to end your current session.</div>
